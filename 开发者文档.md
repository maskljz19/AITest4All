...
Docker Compose 会自动将环境变量传递给所有容器，不需要单独配置 `backend/.env` 和 `frontend/.env`。

### 配置文件对比

| 配置项 | 本地开发 | Docker 部署 |
|-------|---------|------------|
| 数据库地址 | localhost:5432 | postgres:5432 (容器名) |
| Redis地址 | localhost:6379 | redis:6379 (容器名) |
| 前端API地址 | http://localhost:8000 | /api (Nginx代理) |
| 文件路径 | 相对路径 (./uploads) | 容器内绝对路径 (/app/uploads) |
